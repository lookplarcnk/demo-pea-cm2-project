...
function SearchDocumentsPage3() {
  ...
  /**
   * ส่วนที่แก้ไข: เปิดแบบฟอร์มฉบับจริง (Preview) ใน Tab ใหม่
   * ปรับปรุงให้รองรับชื่อไฟล์ภาษาไทยผ่านเส้นทาง /files/
   */
    ...
    <div className="min-h-screen bg-[#F0F4F8] font-sans text-left flex flex-col">
      ...
      <header className="bg-[#00BEEA] py-16 px-4 border-b-4 border-[#00BEEA]">
        <div className="container mx-auto max-w-[1200px] text-center">
          <h2 className="text-2xl md:text-4xl font-black text-white mb-2 uppercase tracking-wide">ค้นหาเอกสารแบบฟอร์ม</h2>
          <p className="text-white opacity-90 text-sm md:text-base font-bold">ศูนย์รวมแบบฟอร์มอิเล็กทรอนิกส์ การไฟฟ้าส่วนภูมิภาคจังหวัดเชียงใหม่ 2</p>
        </div>
      </header>
      ...
      <main className="container mx-auto max-w-[950px] px-4 -mt-12 pb-24 text-left flex-grow">
        <div className="bg-white p-6 md:p-10 rounded-3xl shadow-2xl border border-gray-100 text-left">
          <form className="space-y-8" onSubmit={handleSearchClick}>
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-black text-gray-400 mb-2 uppercase tracking-widest text-left">สถานะแบบฟอร์ม</label>
                <select className="w-full px-4 py-3.5 border border-gray-200 rounded-xl bg-[#F8FAFC] font-bold text-gray-600 outline-none cursor-pointer focus:border-[#00BEEA]">
        ...
        <section className="mt-16 text-left">
          {submittedSearch.trim() || (selectedDept !== "ทั้งหมด" && submittedSearch !== "") ? (
            <div className="results-container">
              <p className="text-gray-400 font-black text-xs uppercase tracking-[0.2em] mb-8 border-l-4 border-[#00BEEA] pl-4 text-left">รายการแบบฟอร์มที่พบ ({filteredDocs.length})</p>
              ...
              {isLoading ? (
                ...
                <div className="space-y-5">
                  {filteredDocs.map((doc) => (
                    <div key={doc.doc_id || doc.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-md hover:shadow-xl hover:border-[#00BEEA] transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group">
                      <div className="flex items-center gap-5 flex-1 cursor-pointer" onClick={() => handleAccess(doc)}>
                        ...
                        <div className="text-left">
                          <h4 className="font-black text-gray-800 text-lg group-hover:text-[#00BEEA] transition-colors flex items-center gap-2">
                            ...
                            {doc.require_login && <span className="text-[10px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-bold">สิทธิ์พนักงานเท่านั้น</span>}
                          </h4>
                          <div className="flex flex-wrap gap-x-5 gap-y-2 mt-2 text-[11px] font-black text-gray-400 uppercase tracking-widest text-left">
                            <span className="text-[#00BEEA] font-bold">{doc.dept || "ทั่วไป"}</span>
                            <span>• {doc.created_at ? new Date(doc.created_at).toLocaleDateString('th-TH') : doc.date}</span>
                            <span>• {doc.file_size || doc.size}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <button type="button" onClick={() => handleAccess(doc)} className="p-3 bg-gray-50 text-gray-400 hover:text-[#00BEEA] hover:bg-[#E0F2FE] rounded-xl transition-all shadow-sm" title="เปิดดูแบบฟอร์มฉบับจริง"><FiExternalLink size={24} /></button>
                      </div>
                    </div>
                  ))}
                  ...
                  {filteredDocs.length === 0 && !isLoading && (
                    <div className="bg-white py-24 rounded-3xl border-2 border-dashed border-gray-200 text-center shadow-inner">
                      <FiSearch className="text-7xl text-gray-100 mx-auto mb-6" />
                      <p className="text-gray-400 font-bold text-xl uppercase tracking-widest">ไม่พบข้อมูลแบบฟอร์มที่ค้นหา</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="bg-blue-50/50 py-20 rounded-3xl border border-blue-100 text-center">
              ...
              <p className="text-blue-400 font-bold uppercase tracking-widest italic">กรุณาระบุคำค้นหาหรือเลือกแผนก และกดปุ่มเพื่อเริ่มค้นหาแบบฟอร์ม</p>
            </div>
          )}
        </section>
      </main>
