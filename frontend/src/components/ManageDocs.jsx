        ...
        <div className="px-4 lg:px-10 pb-10 mt-8 space-y-8 text-left text-left text-left text-left">
          ...
          <div className={`${theme.card} p-5 rounded-[2.5rem] border shadow-sm flex flex-col lg:flex-row gap-5 items-center justify-end text-left text-left`}>
            <div className="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto text-left">
                ...
                <button onClick={() => setOpenAddCategoryModal(true)} className="flex-1 md:flex-none px-5 py-3 bg-purple-50 text-[#74045F] rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-purple-100 transition-all flex items-center justify-center gap-2.5 active:scale-95 text-center text-left text-left"><FiLayers size={16} /> <span className="whitespace-nowrap text-left text-left">จัดการหมวดหมู่</span></button>
                <button onClick={() => setOpenAddDocModal(true)} className="flex-1 md:flex-none px-6 py-3 bg-[#74045F] text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#5a034a] shadow-lg shadow-purple-100 transition-all flex items-center justify-center gap-2.5 active:scale-95 text-center text-left text-left"><FiPlus size={18} /> <span className="whitespace-nowrap text-left text-left">อัปโหลดเอกสาร</span></button>
            </div>
          </div>
            ...
            <div className="overflow-x-auto text-left">
              <table className="w-full min-w-[1000px] text-left">
                <thead className="bg-purple-50/30 text-purple-400 text-[11px] uppercase tracking-widest font-black border-b border-purple-50 text-left">
                  <tr>
                    <th className="px-6 py-5 text-left text-left">ข้อมูลเอกสาร</th>
                    <th className="px-6 py-5 text-left text-left">หมวดหมู่</th>
                    <th className="px-6 py-5 text-left text-left">แผนก</th>
                    <th className="px-6 py-5 text-left text-left">เจ้าของ</th>
                    <th className="px-6 py-5 text-center text-left">สถานะ</th>
                    <th className="px-6 py-5 text-center text-left">วันที่อัปโหลด</th>
                    <th className="px-8 py-5 text-right text-left">การจัดการ</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-purple-50 text-left">
                  {currentItems.map((doc) => (
                    <tr key={doc.doc_id || doc.id} className="hover:bg-purple-50/20 transition-all text-sm font-bold text-left">
                      <td className="px-6 py-5 text-left cursor-pointer text-left" onClick={() => handleViewFile(doc)}>
                        <div className="flex items-center gap-4 text-left">
                      ...
                      <td className="px-6 py-5 text-center text-left">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase inline-flex items-center gap-1.5 text-center text-left ${doc.status === 'อนุมัติแล้ว' ? 'bg-emerald-100 text-emerald-600' : doc.status === 'รออนุมัติ' ? 'bg-amber-100 text-amber-600' : 'bg-rose-100 text-rose-600'}`}>
                          {doc.status === 'อนุมัติแล้ว' ? <FiCheckCircle /> : doc.status === 'รออนุมัติ' ? <FiClock /> : <FiXCircle />} {doc.status}
                        </span>
                      </td>
                      <td className="px-6 py-5 text-center text-slate-400 text-[13px] text-center text-left">
                      ...
                      <td className="px-8 py-5 text-right text-left text-left">
                        <div className="flex justify-end gap-2 text-left text-left">
                          {doc.status === 'รออนุมัติ' && (
                            <>
                              <button 
                                ...
                                className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm flex items-center justify-center text-center text-left" 
                                title="อนุมัติ"
                              >
                                <FiCheck size={16} />
                              </button>
                              <button 
                                ...
                                className="p-2.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm flex items-center justify-center text-center text-left" 
                                title="ปฏิเสธ"
                              >
                                <FiX size={16} />
                              </button>
                          ...
                          <button onClick={() => handleViewFile(doc)} className="p-2.5 bg-slate-100 text-slate-600 rounded-xl hover:bg-[#74045F] hover:text-white transition-all shadow-sm flex items-center justify-center text-center text-left" title="เปิดดูเอกสาร"><FiEye size={16} /></button>
                          <button onClick={() => handleDelete(doc.doc_id || doc.id)} className="p-2.5 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm flex items-center justify-center text-center text-left" title="ลบเอกสาร"><FiTrash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
...
function SummaryCard({ title, value, icon, color }) {
  ...
  return (
    <div className="bg-white border-purple-50 rounded-[2rem] p-6 flex items-center gap-4 border transition-all hover:shadow-xl hover:shadow-[#74045F]/5 group text-left text-left">
      <div className={`p-4 rounded-[1.2rem] text-2xl group-hover:scale-110 transition-transform ${colors[color]} flex items-center justify-center text-center text-left`}>{icon}</div>
...
function AddCategoryModal({ onClose, onSave, categories, onDelete, onEdit }) {
    ...
    // ฟังก์ชันช่วยแสดงระบบยืนยันการลบแบบ Pop-up (ใช้ window.confirm ผสมกับ Toast)
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4 text-left">
            {/* ✅ ปรับขนาด card ให้เล็กลง (max-w-md) */}
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-0 animate-in zoom-in duration-300 font-bold text-left overflow-hidden border border-purple-50">
                <div className="bg-purple-50/30 px-6 py-5 border-b border-purple-50 flex justify-between items-center text-left">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-[#74045F] rounded-xl flex items-center justify-center text-white shadow-lg shadow-purple-100"><FiTag size={18}/></div>
                    <h3 className="text-lg font-black text-slate-800 tracking-tight">จัดการหมวดหมู่</h3>
                  </div>
                  <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-white text-slate-400 hover:text-rose-500 hover:shadow-md rounded-full transition-all border border-purple-50"><FiX size={18}/></button>
                </div>
                <div className="p-6">
                    <div className="mb-6 p-5 bg-purple-50/30 rounded-3xl border border-purple-100/50">
                        <label className="text-[10px] font-black text-[#74045F]/60 uppercase tracking-widest ml-1 mb-2.5 block text-left">เพิ่มหมวดหมู่ใหม่</label>
                        <div className="flex gap-2">
                    ...
                    <div className="space-y-2 max-h-[280px] overflow-y-auto pr-1 custom-scrollbar">
                        {categories.map((cat) => (
                          <div key={cat.cat_id} className="group flex justify-between items-center p-3.5 bg-slate-50/50 rounded-xl border border-transparent hover:border-purple-100 hover:bg-white hover:shadow-md transition-all">
                            {editingId === cat.cat_id ? (
                              <div className="flex gap-2 w-full animate-in slide-in-from-left-2">
                                <input type="text" value={editValue} onChange={(e) => setEditValue(e.target.value)} className="flex-1 bg-white border border-purple-200 rounded-lg px-3 py-1.5 text-xs outline-none focus:ring-4 focus:ring-[#74045F]/5 font-bold" />
                                <button onClick={() => { onEdit(cat.cat_id, editValue); setEditingId(null); }} className="w-8 h-8 flex items-center justify-center bg-emerald-500 text-white rounded-lg shadow-md hover:bg-emerald-600 transition-colors"><FiCheck size={16}/></button>
                                <button onClick={() => setEditingId(null)} className="w-8 h-8 flex items-center justify-center bg-slate-100 text-slate-400 rounded-lg hover:bg-rose-50 hover:text-rose-500 transition-colors"><FiX size={16}/></button>
                              </div>
                            ) : (
                              <>
                                <div className="flex items-center gap-3">
                                  <div className="w-1.5 h-1.5 rounded-full bg-purple-300 group-hover:bg-[#74045F] transition-all"></div>
                                  <span className="text-slate-700 text-xs font-bold tracking-tight">{cat.cat_name}</span>
                                </div>
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                  <button onClick={() => { setEditingId(cat.cat_id); setEditValue(cat.cat_name); }} className="w-7 h-7 flex items-center justify-center text-slate-400 hover:bg-purple-50 hover:text-[#74045F] rounded-lg transition-all"><FiEdit3 size={14}/></button>
                                  <button onClick={() => confirmDelete(cat.cat_id, cat.cat_name)} className="w-7 h-7 flex items-center justify-center text-slate-400 hover:bg-rose-50 hover:text-rose-500 rounded-lg transition-all text-left"><FiTrash2 size={14}/></button>
                                </div>
                              </>
                            )}
                          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4 text-left">
            <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl p-10 animate-in zoom-in duration-300 font-bold relative text-left">
                ...
                <div className="flex flex-col items-center mb-8 text-left text-left">
                  <div className="w-16 h-16 bg-purple-50 text-[#74045F] rounded-3xl flex items-center justify-center mb-4 text-left text-left text-left text-left"><FiUploadCloud size={32} /></div>
                  <h3 className="text-2xl font-black text-slate-800 tracking-tight text-left text-left text-left">อัปโหลดเอกสารส่วนกลาง</h3>
                  <p className="text-slate-400 text-sm font-bold text-left text-left text-left">เลือกไฟล์ PDF ที่ต้องการเผยแพร่เข้าสู่ระบบ</p>
                </div>
                <div className="space-y-6 text-left text-left text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-left text-left text-left">
                      ...
                      <div className="space-y-2 text-left text-left text-left text-left text-left text-left text-left text-left text-left"><label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5 text-left text-left text-left text-left text-left text-left text-left text-left text-left text-left"><FiBriefcase size={12} className="text-left" /> แผนกที่รับผิดชอบ</label>
...
function SidebarItem({ icon, label, active, danger, onClick }) {
  const activeClass = "bg-purple-50 text-[#74045F] shadow-sm shadow-purple-100 text-left font-black text-left text-left text-left";
  const hoverClass = "text-slate-400 hover:bg-purple-50/50 hover:text-[#74045F] text-left font-black text-left text-left text-left";
  return (
    <div onClick={onClick} className={`flex items-center gap-3 px-5 py-4 rounded-2xl cursor-pointer text-sm transition-all text-left text-left text-left text-left text-left ${active ? activeClass : hoverClass} ${danger ? "text-rose-500 mt-auto hover:bg-rose-50" : ""} text-left text-left text-left text-left text-left`}>
      <span className={`flex items-center justify-center text-lg text-center text-left text-left text-left ${active ? 'text-[#74045F]' : 'text-slate-300'}`}>{icon}</span>{label}
    </div>
  );
}
...
function ProfileInput({ label, value, onChange }) {
  return (
    <div className="space-y-1.5 text-left font-bold text-left text-left text-left text-left">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 text-left block">{label}</label>
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-slate-50 border-none rounded-xl px-4 py-3.5 text-slate-700 text-sm focus:ring-4 focus:ring-[#74045F]/10 transition-all outline-none font-bold text-left text-left text-left text-left text-left" />
    </div>
  );
}