      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        <div className={`backdrop-blur-md px-4 lg:px-10 py-6 border-b ${theme.header} sticky top-0 z-30 font-bold flex justify-between items-center`}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl lg:text-2xl font-bold text-slate-800 tracking-tight text-left">จัดการเอกสารทั้งหมด</h2>
          </div>
          <div className="flex items-center gap-4 text-left">
            <button onClick={() => setOpenProfileModal(true)} className="flex-shrink-0 active:scale-95 transition-transform flex items-center justify-center">
              <img src={user.avatar} className="w-11 h-11 rounded-xl object-cover border-2 border-white shadow-md hover:border-indigo-400 transition-all text-left" alt="profile" />
            </button>
          </div>
        </div>
        ...
        <div className="px-4 lg:px-10 pb-10 mt-8 space-y-8 text-left text-left">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 font-bold text-left text-left">
            <SummaryCard title="เอกสารทั้งหมด" value={allDocuments.length} icon={<FiBox />} color="blue" />
          ...
          <div className={`${theme.card} p-5 rounded-[2.5rem] border shadow-sm flex flex-col lg:flex-row gap-5 items-center justify-end text-left`}>
            <div className="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto text-left">
                <div className="relative w-full md:w-72 font-bold text-left">
                  ...
                  <input type="text" placeholder="ค้นหาเอกสาร/แผนก..." className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:bg-white outline-none font-bold text-sm transition-all text-left" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
                <div className="relative w-full md:w-44 font-bold text-left">
                  <FiFilter className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                  <select className="w-full pl-11 pr-10 py-3 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:bg-white outline-none font-bold text-sm appearance-none cursor-pointer transition-all text-left" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
                    <option>ทั้งหมด</option><option>อนุมัติแล้ว</option><option>รออนุมัติ</option><option>ไม่อนุมัติ</option>
                  </select>
                </div>
                <button onClick={() => setOpenAddCategoryModal(true)} className="flex-1 md:flex-none px-5 py-3 bg-indigo-50 text-indigo-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-100 transition-all flex items-center justify-center gap-2.5 active:scale-95 text-left"><FiLayers size={16} /> <span className="whitespace-nowrap">จัดการหมวดหมู่</span></button>
                <button onClick={() => setOpenAddDocModal(true)} className="flex-1 md:flex-none px-6 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all flex items-center justify-center gap-2.5 active:scale-95 text-left"><FiPlus size={18} /> <span className="whitespace-nowrap">อัปโหลดเอกสาร</span></button>
            </div>
          </div>
          ...
          <div className={`${theme.card} rounded-[2.5rem] shadow-sm border overflow-hidden font-bold text-left`}>
            <div className="overflow-x-auto text-left">
              <table className="w-full min-w-[1000px] text-left">
                <thead className="bg-slate-50/50 text-slate-400 text-[11px] uppercase tracking-widest font-black border-b border-slate-50 text-left">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-50 text-left">
                  {currentItems.map((doc) => (
                    <tr key={doc.doc_id || doc.id} className="hover:bg-slate-50/50 transition-all text-sm font-bold text-left">
                      <td className="px-6 py-5 text-left cursor-pointer" onClick={() => handleViewFile(doc)}>
                        <div className="flex items-center gap-4 text-left">
                          <div className="w-11 h-11 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-xl shadow-sm flex-shrink-0 text-left"><FiFileText /></div>
                          <div className="text-left">
                            <p className="text-slate-800 font-black hover:text-indigo-600 transition-colors text-left">{doc.doc_name || doc.name}</p>
                            <p className="text-slate-400 text-[11px] text-left">{doc.file_size || doc.size}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5 text-left">
                        <span className="text-indigo-600 bg-indigo-50 px-3 py-1 rounded-lg text-[10px] font-black uppercase whitespace-nowrap text-left">
                      ...
                      <td className="px-8 py-5 text-right text-left">
                        <div className="flex justify-end gap-2 text-left">
                          ...
                          <button onClick={() => handleViewFile(doc)} className="p-2.5 bg-slate-100 text-slate-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm flex items-center justify-center text-left" title="เปิดดูเอกสาร"><FiEye size={16} /></button>
...
function SummaryCard({ title, value, icon, color }) {
  const colors = { blue: "bg-blue-50 text-blue-600", amber: "bg-amber-50 text-amber-600", purple: "bg-indigo-50 text-indigo-600", rose: "bg-rose-50 text-rose-600" };
  return (
    <div className="bg-white border-slate-100 rounded-[2rem] p-6 flex items-center gap-4 border transition-all hover:shadow-xl hover:shadow-indigo-500/5 group text-left">
...
function AddCategoryModal({ onClose, onSave, categories, onDelete, onEdit }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4 text-left">
            <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl p-0 animate-in zoom-in duration-300 font-bold text-left overflow-hidden border border-slate-100">
                <div className="bg-slate-50/50 px-8 py-6 border-b border-slate-100 flex justify-between items-center text-left">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-100 text-left"><FiTag size={20}/></div>
                    <h3 className="text-xl font-black text-slate-800 tracking-tight text-left">จัดการหมวดหมู่</h3>
                  </div>
                  <button onClick={onClose} className="w-8 h-8 flex items-center justify-center bg-white text-slate-400 hover:text-rose-500 hover:shadow-md rounded-full transition-all border border-slate-100 text-left"><FiX size={18}/></button>
                </div>
                <div className="p-8 text-left">
                    <div className="mb-8 p-6 bg-indigo-50/30 rounded-3xl border border-indigo-100/50 text-left">
                        <label className="text-[11px] font-black text-indigo-400 uppercase tracking-widest ml-1 mb-3 block text-left">เพิ่มหมวดหมู่ใหม่</label>
                        <div className="flex gap-2 text-left">
                          <input autoFocus type="text" value={catName} onChange={(e) => setCatName(e.target.value)} className="flex-1 bg-white text-slate-700 border border-slate-200 rounded-2xl px-5 py-3.5 outline-none font-bold text-sm focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-300 transition-all text-left" placeholder="ระบุชื่อหมวดหมู่..." />
                          <button onClick={() => { if(catName){ onSave(catName); setCatName(""); } }} className="bg-indigo-600 text-white px-5 rounded-2xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center text-left"><FiPlus size={22}/></button>
                        </div>
                    </div>
                    <div className="space-y-2.5 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar text-left">
                        {categories.map((cat) => (
                          <div key={cat.cat_id} className="group flex justify-between items-center p-4 bg-slate-50/50 rounded-2xl border border-transparent hover:border-indigo-100 hover:bg-white hover:shadow-md hover:shadow-indigo-500/5 transition-all text-left">
                            {editingId === cat.cat_id ? (
                              <div className="flex gap-2 w-full animate-in slide-in-from-left-2 text-left">
                                <input type="text" value={editValue} onChange={(e) => setEditValue(e.target.value)} className="flex-1 bg-white border border-indigo-200 rounded-xl px-4 py-2 text-sm outline-none focus:ring-4 focus:ring-indigo-500/10 font-bold text-left" />
                              ...
                              <>
                                <div className="flex items-center gap-3 text-left">
                                  <div className="w-2 h-2 rounded-full bg-indigo-300 group-hover:bg-indigo-500 group-hover:scale-125 transition-all text-left"></div>
                                ...
                                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0 text-left">
                                  <button onClick={() => { setEditingId(cat.cat_id); setEditValue(cat.cat_name); }} className="w-8 h-8 flex items-center justify-center text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg transition-all text-left"><FiEdit3 size={15}/></button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4 text-left">
            <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl p-10 animate-in zoom-in duration-300 font-bold relative text-left">
                ...
                <div className="flex flex-col items-center mb-8 text-left">
                  <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center mb-4 text-left"><FiUploadCloud size={32} /></div>
                ...
                <div className="space-y-6 text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-left">
                      <div className="space-y-2 text-left"><label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5 text-left"><FiTag size={12}/> หมวดหมู่เอกสาร</label>
                        <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-slate-50 text-slate-700 border border-slate-100 rounded-2xl px-5 py-3.5 outline-none font-bold text-sm appearance-none cursor-pointer hover:bg-white transition-all text-left">{categories.map(c => <option key={c.cat_id} value={c.cat_name}>{c.cat_name}</option>)}</select>
                      </div>
                      <div className="space-y-2 text-left"><label className="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5 text-left"><FiBriefcase size={12}/> แผนกที่รับผิดชอบ</label>
                        <select value={dept} onChange={(e) => setDept(e.target.value)} className="w-full bg-slate-50 text-slate-700 border border-slate-100 rounded-2xl px-5 py-3.5 outline-none font-bold text-sm appearance-none cursor-pointer hover:bg-white transition-all text-left">{departments.map(d => <option key={d} value={d}>{d}</option>)}</select>
                      </div>
                    </div>
                    <button disabled={!selectedFile} onClick={() => onSave({ name, size: `${(selectedFile.size / (1024 * 1024)).toFixed(2)} MB`, category, dept })} className={`w-full py-4 rounded-2xl font-black uppercase text-xs tracking-widest transition-all shadow-lg flex items-center justify-center gap-2 ${selectedFile ? 'bg-indigo-600 text-white shadow-indigo-100 hover:bg-indigo-700 active:scale-95' : 'bg-slate-100 text-slate-300 cursor-not-allowed'} text-left`}><FiCheckCircle size={18} /> เริ่มการอัปโหลด</button>
                </div>
            </div>
        </div>
    );
...
function SidebarItem({ icon, label, active, danger, onClick }) {
  ...
  return (
    <div onClick={onClick} className={`flex items-center gap-3 px-5 py-4 rounded-2xl cursor-pointer text-sm font-black transition-all text-left ${active ? activeClass : hoverClass} ${danger ? "text-rose-500 mt-auto hover:bg-rose-50" : ""} text-left text-left`}>
      <span className="flex items-center justify-center text-lg text-left text-left">{icon}</span>{label}
    </div>
  );
}
...
function ProfileInput({ label, value, onChange }) {
  return (
    <div className="space-y-1.5 text-left font-bold text-left text-left text-left">
      ...
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-slate-50 border-none rounded-xl px-4 py-3.5 text-slate-700 text-sm focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none font-bold text-left text-left text-left" />
    </div>
  );
}