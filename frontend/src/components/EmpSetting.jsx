...
export default function EmpSetting() {
  ...
  // ✅ 3. ฟังก์ชันบันทึกการตั้งค่าพร้อม Toast (ใช้โทนสี Indigo)
  const handleSaveSettings = () => {
    const loadToast = toast.loading("กำลังบันทึกการตั้งค่า...");
    setTimeout(() => {
        if (settings.notifications) {
            toast.success('บันทึกการตั้งค่าเรียบร้อยแล้ว', { id: loadToast });
          } else {
  ...
  return (
    <div className={`flex min-h-screen ${theme.bg} font-sans ${theme.textMain} overflow-x-hidden text-left font-medium`}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 ${theme.sidebar} border-r flex flex-col transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className={`p-6 border-t border-slate-50 space-y-2 font-bold text-left text-left`}>
      ...
      <main className="flex-1 min-w-0 overflow-y-auto text-left">
        <div className={`backdrop-blur-md px-4 lg:px-10 py-6 border-b ${theme.header} sticky top-0 z-30 font-bold flex justify-between items-center text-left text-left`}>
          <div className="flex items-center gap-3 text-left">
            <button onClick={() => setIsSidebarOpen(true)} className={`p-2.5 bg-white border-slate-200 rounded-xl shadow-sm border lg:hidden text-slate-600 flex items-center justify-center text-left`}><FiMenu size={20} /></button>
            <h2 className={`text-2xl lg:text-3xl font-bold text-slate-800 tracking-tight text-left`}>ตั้งค่า</h2>
          </div>
          ...
          <div className="flex items-center gap-3 text-left">
            <div className="text-right hidden sm:block text-left">
              ...
              <p className="text-[12px] font-bold text-[#A855F7] mt-0.5 text-left">{user.department}</p>
            </div>
        ...
        <div className="px-4 lg:px-10 pb-10 mt-8 space-y-8 max-w-5xl mx-auto text-left text-left">
          {/* ✅ เปลี่ยนสีไอคอนเป็น Indigo แบบ AdminSetting */}
          <section className="space-y-4 text-left">
            <div className="flex items-center gap-3 mb-6 text-left">
              <div className={`w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center text-left`}><FiBell size={20} /></div>
              <h3 className={`text-xl font-black text-slate-800 text-left`}>การแจ้งเตือน</h3>
            </div>
          ...
          {/* ✅ เปลี่ยนสีไอคอนเป็น Rose/Indigo แบบ AdminSetting */}
          ...
          <section className="space-y-4 text-left text-left">
            <div className="flex items-center gap-3 mb-6 text-left">
              <div className={`w-10 h-10 bg-amber-50 text-amber-600 rounded-xl flex items-center justify-center text-left`}><FiSettings size={20} /></div>
            ...
            <div className={`${theme.card} p-8 rounded-[2.5rem] border shadow-sm space-y-6 text-left`}>
               ...
               <div className="flex gap-4 text-left">
                  {/* ✅ เปลี่ยนสีปุ่มบันทึกเป็น Indigo-600 แบบ AdminSetting */}
                  <button onClick={handleSaveSettings} className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95 text-sm uppercase tracking-widest flex items-center gap-2 text-left">
                    <FiCheck size={18}/> บันทึกการตั้งค่า
                  </button>
                  <button onClick={() => navigate("/EmployeeDashboard")} className="bg-white border border-slate-200 text-slate-500 px-8 py-3 rounded-2xl font-black hover:bg-slate-50 transition-all active:scale-95 text-sm uppercase tracking-widest text-left">
                    ยกเลิก
                  </button>
               </div>
            </div>
  ...
  return (
    <div className={`bg-white border-slate-100 hover:border-indigo-100 p-6 rounded-[2rem] border shadow-sm flex items-center justify-between group transition-all text-left`}>
      <div className="flex items-center gap-5 text-left">
        <div className={`w-12 h-12 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-all shadow-sm text-left`}>{icon}</div>
      ...
      {/* ✅ เปลี่ยนสี Toggle เป็น Indigo-600 แบบ AdminSetting */}
      <button onClick={onToggle} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none ${active ? 'bg-indigo-600' : 'bg-slate-200'} text-left`}>
...
function SidebarItem({ icon, label, active, danger, onClick }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-3 px-5 py-4 rounded-2xl cursor-pointer text-sm font-black transition-all text-left text-left ${active ? "bg-purple-50 text-purple-700 shadow-sm shadow-purple-100" : "text-slate-400 hover:bg-slate-50 hover:text-slate-700"} ${danger ? "text-rose-500 mt-auto text-left" : ""} text-left`}>
      <span className={`${active ? "text-purple-600 text-lg" : "text-slate-300 text-lg"} flex items-center justify-center text-left text-left`}>{icon}</span>{label}
    </div>
  );
}