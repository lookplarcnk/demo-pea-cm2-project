    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4 font-bold text-left animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-10 relative animate-in zoom-in duration-300">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button onClick={onClose} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-500 font-black py-4 rounded-2xl transition-all active:scale-95 uppercase tracking-widest text-xs text-center">ยกเลิก</button>
        </div>
      ...
      <main className="flex-1 min-w-0 overflow-y-auto text-left">
        <div className="bg-white/70 backdrop-blur-md px-4 lg:px-10 py-6 border-b border-purple-50 sticky top-0 z-30 font-bold text-left">
          <div className="flex items-center justify-between text-left">
            <div className="flex items-center gap-3 text-left">
              <button onClick={() => setIsSidebarOpen(true)} className="p-2.5 bg-white rounded-xl shadow-sm border border-purple-100 lg:hidden text-[#74045F] flex items-center justify-center text-left"><FiMenu size={20} /></button>
            ...
            <div className="flex items-center gap-3 text-left">
              <button onClick={() => setOpenProfileModal(true)} className="active:scale-95 transition-transform text-left">
                  <img src={user.avatar} className="w-11 h-11 rounded-xl object-cover border-2 border-white shadow-md hover:border-[#74045F] text-left" alt="profile" />
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 lg:px-10 pb-10 mt-8 text-left">
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-8 items-end bg-white p-6 rounded-[2.5rem] border border-purple-50 shadow-sm transition-all font-bold text-left">
            <FilterSelect label="สถานะคำขอของ Admin" value={activeTab} onChange={setActiveTab} options={["ทั้งหมด", "รออนุมัติ", "อนุมัติแล้ว", "ไม่อนุมัติ"]} icon={<FiLayers className="text-[#74045F]"/>} />
            <div className="md:col-span-2 space-y-2.5 text-left text-base">
              <label className="text-xs font-bold text-[#74045F]/60 uppercase tracking-widest ml-1 font-black text-left">ค้นหาประวัติการส่ง</label>
              <div className="relative group flex gap-2 text-left">
                <div className="relative flex-1 text-left">
                  ...
                  <input type="text" placeholder="ระบุชื่อเอกสารหรือผู้รับพิจารณา..." className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border-none rounded-2xl text-base font-semibold outline-none transition-all focus:ring-2 focus:ring-purple-100 text-left" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={handleKeyDown} />
                </div>
                <button onClick={handleSearchClick} className="bg-[#74045F] hover:bg-[#5a034a] text-white px-6 rounded-2xl font-bold transition-all shadow-lg active:scale-95 flex items-center justify-center text-left"><FiSearch size={20} /></button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-purple-50 overflow-hidden font-bold text-left">
            <div className="px-8 py-6 border-b border-purple-50 bg-purple-50/20 flex justify-between items-center text-left">
                <h3 className="font-bold text-[#74045F] text-lg tracking-tight text-left">รายการคำขอที่ Admin ยื่น</h3>
                <button onClick={() => setIsUploadModalOpen(true)} className="flex items-center gap-2 bg-[#74045F] hover:bg-[#5a034a] text-white px-6 py-3 rounded-2xl font-black text-sm transition-all shadow-lg active:scale-95 text-left"><FiPlus size={20}/> สร้างคำขอใหม่</button>
            </div>
                ...
                <tbody className="divide-y divide-purple-50 font-bold text-left">
                  {currentItems.map((doc, index) => (
                    <tr key={doc.id} className="hover:bg-purple-50/20 transition-colors group font-semibold text-sm lg:text-base text-left">
                      <td className="px-6 py-5 text-center font-bold text-purple-200 text-left">{indexOfFirstItem + index + 1}</td>
                      <td className="px-4 py-5 font-bold text-slate-700 text-left">
                          <div className="flex items-center gap-4 text-left">
                            <div className="w-10 h-10 bg-purple-50 text-[#74045F] rounded-xl flex items-center justify-center flex-shrink-0 font-bold text-left"><FiFileText size={20} /></div>
                            <div className="text-left">
                              <p className="font-bold text-slate-700 line-clamp-1 text-left">{doc.name}</p>
                              <p className="text-[11px] text-[#74045F]/40 font-bold uppercase tracking-tighter text-left">{doc.category} • {doc.date}</p>
                            </div>
                          </div>
                      </td>
                      ...
                      <td className="px-8 py-5 text-right font-black text-left">
                        <button onClick={() => setViewingDoc(doc)} className="w-11 h-11 bg-purple-50 text-[#74045F] hover:bg-[#74045F] hover:text-white rounded-xl shadow-sm transition-all active:scale-90 flex items-center justify-center ml-auto text-left"><FiEye size={22}/></button>
                      </td>
                    </tr>
                  ))}
                    ...
                    <tr>
                      <td colSpan="5" className="px-6 py-24 text-center text-slate-400 font-bold italic bg-slate-50/10">
                        <div className="flex flex-col items-center justify-center gap-3">
                           <FiFileText size={48} className="opacity-20 text-[#74045F]" />
            ...
            <div className="px-8 py-6 border-t border-purple-50 flex flex-col sm:flex-row items-center justify-between gap-4 bg-purple-50/10 font-bold text-left">
              ...
              <div className="flex items-center gap-1 font-bold text-left">
                <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className={`p-2 rounded-lg transition-all flex items-center justify-center text-left ${currentPage === 1 ? "text-purple-100 cursor-not-allowed" : "text-[#74045F] hover:bg-purple-50"}`}><FiChevronLeft size={20} /></button>
                <div className="flex items-center gap-1 text-left">
                  {[...Array(totalPages)].map((_, i) => (
                    <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`min-w-[36px] h-9 px-2 rounded-lg text-sm font-black transition-all flex items-center justify-center text-left ${currentPage === i + 1 ? "bg-[#74045F] text-white shadow-lg shadow-purple-100" : "text-slate-400 hover:bg-purple-50"}`}>{i + 1}</button>
                  ))}
                </div>
                <button disabled={currentPage === totalPages || filteredDocs.length === 0} onClick={() => setCurrentPage(prev => prev + 1)} className={`p-2 rounded-lg transition-all flex items-center justify-center text-left ${currentPage === totalPages || filteredDocs.length === 0 ? "text-purple-100 cursor-not-allowed" : "text-[#74045F] hover:bg-purple-50"}`}><FiChevronRight size={20} /></button>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300 text-left">
      <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-10 relative flex flex-col font-bold max-h-[90vh] overflow-y-auto text-left border border-purple-50">
        ...
        <h3 className="text-2xl font-black text-[#74045F] mb-2 tracking-tight text-left">สร้างคำขอพิจารณาใหม่ (Admin)</h3>
        ...
        <div className="space-y-6 text-left">
          <div className="space-y-1 relative text-left">
            <label className="text-xs font-black text-purple-300 uppercase tracking-widest ml-1 flex items-center gap-2 text-left"><FiUser className="text-[#74045F]"/> ค้นหาผู้อนุมัติปลายทาง <span className="text-rose-500">*</span></label>
            <input type="text" placeholder="ระบุชื่อผู้อนุมัติ..." value={approverSearch} 
              ...
              className="w-full bg-slate-50 border-none rounded-xl pl-6 py-3.5 font-bold outline-none focus:ring-2 focus:ring-[#74045F] transition-all text-left" 
            />
            {showApproverList && filteredApprovers.length > 0 && (
              <div className="absolute z-[110] w-full mt-2 bg-white rounded-2xl shadow-xl border border-purple-50 overflow-hidden max-h-60 overflow-y-auto text-left">
                {filteredApprovers.map(emp => (
                  <div key={emp.emp_id} onClick={() => { setFormData(prev => ({...prev, approver: emp.emp_name, approverId: emp.emp_id})); setApproverSearch(emp.emp_name); setShowApproverList(false); }} className="px-5 py-3 hover:bg-purple-50 cursor-pointer border-b border-purple-50 last:border-none text-left text-slate-700">
                    <p className="text-sm font-black text-left">{emp.emp_name}</p>
                    <p className="text-[10px] text-[#74045F]/50 font-bold uppercase text-left">{emp.emp_dept || "บุคลากร"}</p>
                  </div>
                ))}
              </div>
            )}
          </div>
          <div className={`border-2 border-dashed rounded-[2rem] p-10 flex flex-col items-center justify-center transition-all relative text-left ${formData.file ? 'border-[#74045F] bg-purple-50' : 'border-purple-100 bg-slate-50'}`} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); handleFileChange(e.dataTransfer.files[0]); }}>
            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 text-left"><FiUpload size={28} className="text-[#74045F] text-left" /></div>
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300 text-left">
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row font-bold text-left border border-purple-50">
        <div className="flex-[1.5] bg-slate-200 p-8 overflow-hidden flex flex-col relative text-left">
          <iframe src={`${doc.fileUrl}#toolbar=0`} className="w-full h-full rounded-2xl border-none text-left" title="Preview" />
        </div>
        <div className="flex-1 bg-white p-12 flex flex-col border-l border-purple-50 overflow-y-auto text-left">
          <button onClick={onClose} className="self-end w-10 h-10 text-slate-300 hover:text-[#74045F] transition-all mb-6 flex items-center justify-center bg-purple-50 rounded-full text-left"><FiX size={24} /></button>
          <div className="mb-8 text-left"><span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase text-left ${doc.status === 'อนุมัติแล้ว' ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>{doc.status}</span><h3 className="text-2xl font-black text-[#74045F] mt-2 text-left">{doc.name}</h3></div>
          <div className="space-y-6 flex-1 text-left"><div className="grid grid-cols-2 gap-4 text-left"><DetailItem label="วันที่ส่ง" value={doc.date} /><DetailItem label="ขนาด" value={doc.size} /></div><DetailItem label="ผู้รับพิจารณา" value={doc.approver} /><div className="p-6 bg-purple-50/30 rounded-3xl space-y-4 text-left"><h5 className="text-xs font-black text-[#74045F]/60 uppercase tracking-widest text-left">ข้อมูลเพิ่มเติม</h5><p className="text-sm text-slate-600 italic text-left">"{doc.description || 'ไม่มีข้อมูลเพิ่มเติม'}"</p></div></div>
          <button onClick={onClose} className="mt-10 w-full bg-[#74045F] text-white py-4 rounded-2xl font-black uppercase text-center active:scale-95 transition-all">ปิดหน้าต่าง</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-2 text-left font-bold">
      ...
      <div className="relative text-left">
        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-purple-300 text-left">{icon}</span>
        <select value={value} onChange={e => onChange(e.target.value)} className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border-none rounded-2xl text-base font-bold outline-none appearance-none cursor-pointer text-left focus:ring-2 focus:ring-purple-100">{options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>
      </div>
    </div>
  );
}
...
function SidebarItem({ icon, label, active, danger, onClick }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-3 px-5 py-3 rounded-2xl cursor-pointer transition-all text-left ${active ? "bg-purple-50 text-[#74045F] shadow-sm shadow-purple-100 font-black" : "text-slate-400 hover:bg-purple-50 hover:text-[#74045F] font-black" } ${danger ? "text-rose-500 hover:bg-rose-50 mt-auto font-black" : ""}`}>
      <span className={active ? "text-[#74045F] text-lg" : "text-purple-200 text-lg"}>{icon}</span><span className="text-[14px]">{label}</span>
    </div>
  );
}
...
function ProfileInput({ label, value, onChange }) {
  return (
    <div className="space-y-1 text-left block font-bold">
      ...
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-slate-700 text-sm focus:ring-4 focus:ring-purple-100 transition-all outline-none font-bold text-left" />
    </div>
  );
}