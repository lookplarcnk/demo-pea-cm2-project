...
export default function AdminSubmitApproval() {
  ...
  const fetchAdminSubmittedDocuments = async () => {
    try {
      // ✅ แก้ไข: ดึงข้อมูลประวัติการส่งผ่าน Render
      ...
      if (res.data) {
        const formattedDocs = res.data
          ...
          .map(doc => ({
            ...
            // ✅ แก้ไข: ใช้ SERVER_URL (Render) สำหรับ URL ไฟล์
  ...
  const handleUploadSubmit = async (newDoc) => {
    ...
    try {
      ...
      // ✅ แก้ไข: ส่งข้อมูลไปยัง Endpoint บน Render
      ...
      <main className="flex-1 min-w-0 overflow-y-auto text-left">
        <div className="bg-white/70 backdrop-blur-md px-4 lg:px-10 py-6 border-b border-purple-50 sticky top-0 z-30 font-bold text-left">
          <div className="flex items-center justify-between text-left">
            <div className="flex items-center gap-3 text-left">
              ...
              <h2 className="text-2xl lg:text-3xl font-bold text-[#74045F] tracking-tight text-left text-left">ส่งเอกสารให้พิจารณา</h2>
            </div>
            <div className="flex items-center gap-3 text-left">
        ...
        <div className="px-4 lg:px-10 pb-10 mt-8 text-left">
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-8 items-end bg-white p-6 rounded-[2.5rem] border border-purple-50 shadow-sm transition-all font-bold text-left">
            <FilterSelect label="สถานะคำขอของ Admin" value={activeTab} onChange={setActiveTab} options={["ทั้งหมด", "รออนุมัติ", "อนุมัติแล้ว", "ไม่อนุมัติ"]} icon={<FiLayers className="text-[#74045F] text-left"/>} />
            <div className="md:col-span-2 space-y-2.5 text-left text-base text-left">
              <label className="text-xs font-bold text-[#74045F]/60 uppercase tracking-widest ml-1 font-black text-left block">ค้นหาประวัติการส่ง</label>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-purple-50 overflow-hidden font-bold text-left">
            <div className="px-8 py-6 border-b border-purple-50 bg-purple-50/20 flex justify-between items-center text-left">
                ...
                <button onClick={() => setIsUploadModalOpen(true)} className="flex items-center gap-2 bg-[#74045F] hover:bg-[#5a034a] text-white px-6 py-3 rounded-2xl font-black text-sm transition-all shadow-lg active:scale-95 text-center"><FiPlus size={20}/> สร้างคำขอใหม่</button>
            </div>
              ...
              <table className="w-full text-left border-collapse font-bold text-left">
                <thead className="bg-white border-b border-purple-50 text-[#74045F]/60 text-[13px] uppercase font-black text-left">
                  <tr>
                    ...
                    <th className="px-4 py-5 font-black text-left text-left">รายการเอกสาร</th>
                    <th className="px-6 py-5 font-black text-left text-left">ส่งถึงผู้อนุมัติ</th>
                    <th className="px-6 py-5 text-center font-black text-left text-center">สถานะ</th>
                    <th className="px-8 py-5 text-right font-black text-left text-right">รายละเอียด</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-purple-50 font-bold text-left">
                  {currentItems.map((doc, index) => (
                    <tr key={doc.id} className="hover:bg-purple-50/20 transition-colors group font-semibold text-sm lg:text-base text-left">
                      ...
                      <td className="px-6 py-5 text-center font-bold text-center">
                      ...
                      <td className="px-8 py-5 text-right font-black text-right">
                        <button onClick={() => setViewingDoc(doc)} className="w-11 h-11 bg-purple-50 text-[#74045F] hover:bg-[#74045F] hover:text-white rounded-xl shadow-sm transition-all active:scale-90 flex items-center justify-center ml-auto text-center"><FiEye size={22}/></button>
                      </td>
                    </tr>
                  ))}
                  {currentItems.length === 0 && (
                    <tr>
                      <td colSpan="5" className="px-6 py-24 text-center text-slate-400 font-bold italic bg-slate-50/10 text-center">
                        <div className="flex flex-col items-center justify-center gap-3 text-center">
                           <FiFileText size={48} className="opacity-20 text-[#74045F] text-center" />
                           <span className="text-base text-center">ไม่พบประวัติการส่งเอกสารของ Admin</span>
                        </div>
                      </td>
                    </tr>
                  )}
            ...
            <div className="px-8 py-6 border-t border-purple-50 flex flex-col sm:flex-row items-center justify-between gap-4 bg-purple-50/10 font-bold text-left">
              ...
              <div className="flex items-center gap-1 font-bold text-left text-center">
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300 text-left">
      <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-10 relative flex flex-col font-bold max-h-[90vh] overflow-y-auto text-left border border-purple-50">
        <button onClick={onClose} className="absolute right-8 top-8 text-slate-300 hover:text-rose-50 transition-all text-center"><FiX size={24} /></button>
        ...
        <div className="space-y-6 text-left">
          <div className="space-y-1 relative text-left text-left">
            <label className="text-xs font-black text-purple-300 uppercase tracking-widest ml-1 flex items-center gap-2 text-left"><FiUser className="text-[#74045F] text-left"/> ค้นหาผู้อนุมัติปลายทาง <span className="text-rose-500">*</span></label>
            <input type="text" placeholder="ระบุชื่อผู้อนุมัติ..." value={approverSearch} 
              ...
              className="w-full bg-slate-50 border-none rounded-xl pl-6 py-3.5 font-bold outline-none focus:ring-2 focus:ring-[#74045F] transition-all text-left text-slate-700" 
            />
          ...
          <div className={`border-2 border-dashed rounded-[2rem] p-10 flex flex-col items-center justify-center transition-all relative text-left ${formData.file ? 'border-[#74045F] bg-purple-50' : 'border-purple-100 bg-slate-50'}`} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); handleFileChange(e.dataTransfer.files[0]); }}>
            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-4 text-center text-left"><FiUpload size={28} className="text-[#74045F] text-center text-left" /></div>
            <p className="text-slate-600 font-bold text-center text-left">{formData.file ? formData.file.name : "เลือกหรือลากไฟล์ PDF มาวางที่นี่ *"}</p>
...
function DocumentDetailModal({ doc, onClose }) {
  return (
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300 text-left">
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row font-bold text-left border border-purple-50 text-left">
  ...
  return (
    <div className="space-y-2 text-left font-bold text-left">
      ...
      <div className="relative text-left">
        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-purple-300 text-center text-left">{icon}</span>
        <select value={value} onChange={e => onChange(e.target.value)} className="w-full pl-11 pr-4 py-3.5 bg-slate-50 border-none rounded-2xl text-base font-bold outline-none appearance-none cursor-pointer text-left focus:ring-2 focus:ring-purple-100 text-slate-700">{options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>
      </div>
    </div>
  );
}
...
function SidebarItem({ icon, label, active, danger, onClick }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-3 px-5 py-3 rounded-2xl cursor-pointer transition-all text-left ${active ? "bg-purple-50 text-[#74045F] shadow-sm shadow-purple-100 font-black" : "text-slate-400 hover:bg-purple-50 hover:text-[#74045F] font-black" } ${danger ? "text-rose-500 hover:bg-rose-50 mt-auto font-black text-left" : ""}`}>
      <span className={active ? "text-[#74045F] text-lg text-center" : "text-purple-200 text-lg text-center"}>{icon}</span><span className="text-[14px] text-left">{label}</span>
    </div>
  );
}
...
function ProfileInput({ label, value, onChange }) {
  return (
    <div className="space-y-1 text-left block font-bold text-left">
      <label className="text-[10px] font-black text-purple-300 uppercase tracking-widest ml-1 font-black text-left block">{label}</label>
