...
function SearchDocumentsPage4() {
  ...
  const [hasSearched, setHasSearched] = useState(false); // ควบคุมการแสดงผลลัพธ์
  
  const navigate = useNavigate();
  const user = JSON.parse(localStorage.getItem("user"));

  // ดึงข้อมูลแผนกจาก Database สำหรับ Dropdown
  useEffect(() => {
  ...
  // ฟังก์ชันค้นหาเอกสาร (ล็อค cat_id = 7 และจะทำงานเมื่อกด Submit เท่านั้น)
  const handleSearch = async (e) => {
    if (e) e.preventDefault();
    setLoading(true);
    setHasSearched(true); // เริ่มการแสดงผลลัพธ์
    try {
      const response = await axios.get(`${API_BASE_URL}/public/documents/procurement`, { 
        params: {
          query: searchQuery,
  ...
  const handleAccess = (doc) => {
    ...
    const fileUrl = `http://localhost:5000/files/${fileName}.pdf`;
    ...
    <div className="min-h-screen bg-[#F0F4F8] font-sans text-left flex flex-col">
      ...
      <header className="bg-[#00BEEA] py-16 px-4 border-b-4 border-[#00BEEA]">
        <div className="container mx-auto max-w-[1200px] text-center">
          <h2 className="text-2xl md:text-4xl font-black text-white mb-2 uppercase tracking-wide">ค้นหาเอกสารการจัดซื้อจัดจ้าง</h2>
          <p className="text-white opacity-90 text-sm md:text-base font-bold">ระบบข้อมูลข่าวสารการพัสดุและการจัดหาพัสดุ การไฟฟ้าส่วนภูมิภาคจังหวัดเชียงใหม่ 2</p>
        </div>
      </header>
      ...
      <main className="container mx-auto max-w-[950px] px-4 -mt-12 pb-24 text-left flex-grow">
        <div className="bg-white p-6 md:p-10 rounded-3xl shadow-2xl border border-gray-100 text-left">
          <form className="space-y-8" onSubmit={handleSearch}>
            <div>
              <label className="block text-xs font-black text-gray-400 mb-2 uppercase tracking-widest text-left">ค้นหาประกาศจัดซื้อจัดจ้าง / เลขที่โครงการ</label>
              <input
                type="text"
          ...
          {hasSearched ? (
            documents.length > 0 ? (
              <div className="results-container">
                <p className="text-gray-400 font-black text-xs uppercase tracking-[0.2em] mb-8 border-l-4 border-[#00BEEA] pl-4 text-left">รายการประกาศที่เกี่ยวข้อง ({documents.length})</p>
                      ...
                      <div className="flex items-center gap-5 flex-1 cursor-pointer" onClick={() => handleAccess(doc)}>
                        ...
                        <div className="text-left">
                          ...
                          <div className="flex flex-wrap gap-x-5 gap-y-2 mt-2 text-[11px] font-black text-gray-400 uppercase tracking-widest text-left">
                            <span className="text-[#00BEEA]">{doc.category || "ประกาศจัดซื้อ"}</span>
              ...
              <div className="bg-blue-50/50 py-20 rounded-3xl border border-blue-100 text-center">
                ...
                <p className="text-blue-400 font-bold uppercase tracking-widest italic">ไม่พบประกาศที่คุณต้องการ</p>
              </div>
            )
          ) : (
...
export default SearchDocumentsPage4;