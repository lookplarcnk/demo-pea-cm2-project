      ...
      <main className="flex-1 min-w-0 overflow-y-auto text-left">
        <div className="bg-white/70 backdrop-blur-md px-4 lg:px-10 py-6 border-b border-purple-50 sticky top-0 z-30 font-bold flex justify-between items-center text-left">
          <div className="flex items-center gap-3 text-left">
            ...
            <h2 className="text-2xl lg:text-3xl font-bold text-[#74045F] tracking-tight text-left">จัดการเอกสาร</h2>
          </div>
          <div className="flex items-center gap-3 text-left">
        ...
        <div className="px-4 lg:px-10 pb-10 mt-8 text-left">
          {latestAnnouncement && (
            <div className="mb-8 bg-gradient-to-r from-purple-50 to-amber-50 border border-purple-100 rounded-[2rem] p-6 flex items-center gap-5 animate-in slide-in-from-top duration-500 text-left">
              <div className="w-12 h-12 bg-[#74045F] text-white rounded-2xl flex items-center justify-center shadow-lg shadow-purple-200 flex-shrink-0 text-left">
                <FiAlertCircle size={24} />
              </div>
              <div className="flex-1 text-left">
                <div className="flex items-center gap-2 mb-0.5 text-left">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#74045F]">ประกาศแจ้งเตือนระบบ</span>
              ...
              <button onClick={() => setLatestAnnouncement(null)} className="p-2 hover:bg-purple-100 rounded-full text-purple-400 transition-all text-left">
                <FiX size={20} />
              </button>
            </div>
          )}
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-purple-50 overflow-hidden font-bold text-left">
            <div className="p-6 lg:p-8 flex justify-between items-center border-b border-purple-50 bg-purple-50/20 text-left">
              <h3 className="font-bold text-[#74045F] text-lg uppercase tracking-tight text-left">คลังข้อมูลส่วนตัว</h3>
              <div className="flex items-center gap-4 text-left">
                <div className="relative hidden md:block group font-bold text-left">
                    <FiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-purple-300 text-left" />
                    <input type="text" placeholder="ค้นหาเอกสารของคุณ..." className="pl-11 pr-4 py-2.5 rounded-2xl bg-white border border-purple-100 focus:ring-4 focus:ring-[#74045F]/5 outline-none transition-all w-64 font-medium text-left" value={searchQuery} onChange={(e) => {setSearchQuery(e.target.value); setCurrentPage(1);}} />
                </div>
                <button onClick={() => { setEditingDoc(null); setOpenUploadModal(true); }} className="flex items-center justify-center gap-2 bg-[#74045F] hover:bg-[#5a034a] text-white px-6 py-3 rounded-2xl font-bold transition-all shadow-lg shadow-purple-100 active:scale-95 text-base uppercase tracking-wider text-left">
            ...
            <div className="overflow-x-auto text-left">
              <table className="w-full min-w-[700px] text-left font-bold">
                <thead className="bg-white text-[#74045F]/40 text-[16px] uppercase tracking-widest font-black border-b border-purple-50 text-left">
                  <tr>
                ...
                <tbody className="divide-y divide-purple-50 font-bold text-left">
                  {currentItems.length > 0 ? currentItems.map((doc, index) => (
                    <tr key={doc.doc_id || doc.id} className="hover:bg-purple-50/20 transition-all font-bold text-left">
                      <td className="px-6 py-6 text-center text-purple-200 font-black">{indexOfFirstItem + index + 1}</td>
                      <td className="px-8 py-6 text-left text-left">
                        <div className="flex items-center gap-4 text-left">
                          <div className="w-12 h-12 bg-purple-50 text-[#74045F] rounded-xl flex items-center justify-center text-xl font-bold flex-shrink-0 text-left"><FiFileText /></div>
                          <span className="text-slate-700 line-clamp-1 font-bold text-xl text-left">{doc.doc_name || doc.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-6 text-left">
                        <span className="bg-purple-50 text-[#74045F] px-4 py-1.5 rounded-xl text-[13px] font-black uppercase text-left">{doc.category || "ไม่ระบุ"}</span>
                      </td>
                      ...
                      <td className="px-8 py-6 text-right font-black text-left">
                        <div className="flex justify-end gap-2 text-left text-left">
                          <button onClick={() => { setEditingDoc(doc); setOpenUploadModal(true); }} className="p-3 bg-purple-50 text-[#74045F] rounded-xl hover:bg-[#74045F] hover:text-white transition-all shadow-sm active:scale-90 font-bold flex items-center justify-center text-left text-left"><FiEdit size={18} /></button>
            ...
            <div className="px-8 py-10 border-t border-purple-50 bg-purple-50/10 font-bold text-left flex items-center justify-between text-left">
              ...
              <div className="flex items-center gap-2 text-left">
                <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => prev - 1)} className={`w-10 h-10 rounded-xl transition-all flex items-center justify-center shadow-sm text-left ${currentPage === 1 ? "bg-white text-slate-200 cursor-not-allowed" : "bg-white text-[#74045F] hover:bg-purple-50 active:scale-90"}`}>
                    <FiChevronLeft size={20} />
                </button>
                <div className="flex items-center gap-2 px-2 text-left text-left">
                  {[...Array(totalPages)].map((_, i) => (
                    <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`w-10 h-10 rounded-xl font-black text-sm transition-all flex items-center justify-center shadow-sm text-left ${currentPage === i + 1 ? "bg-[#74045F] text-white shadow-purple-200 scale-110" : "bg-white text-[#74045F] hover:bg-purple-50 active:scale-95"}`}>
                      {i + 1}
                    </button>
                  ))}
                </div>
...
function SidebarItem({ icon, label, active, danger, onClick }) {
  return (
    <div 
      onClick={onClick}
      className={`flex items-center gap-3 px-5 py-3 rounded-2xl cursor-pointer text-sm font-black transition-all text-left text-left ${active ? "bg-purple-50 text-[#74045F] shadow-sm shadow-purple-100" : "text-slate-400 hover:bg-purple-50 hover:text-[#74045F]"} ${danger ? "text-rose-500 mt-auto hover:bg-rose-50" : ""}`}
    >
      <span className={`${active ? "text-[#74045F] text-lg" : "text-purple-200 text-lg"} flex items-center justify-center text-left text-left`}>{icon}</span>
      {label}
    </div>
  );
}
...
function SummaryCard({ title, value, icon, color }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] p-8 flex items-center gap-6 border border-purple-50 transition-all hover:shadow-xl hover:shadow-purple-500/5 group text-left text-left">
      ...
      <div className="text-left font-bold text-left">
        <p className="text-[14px] font-black text-[#74045F]/40 uppercase tracking-widest mb-1 text-left">{title}</p>
...
function UploadModal({ onClose, onSave, editingDoc, categories }) {
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-300 text-left text-left text-left">
        <div className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-10 relative animate-in zoom-in duration-200 font-bold text-left text-left text-left border border-purple-50">
          <button onClick={onClose} className="absolute right-8 top-8 w-10 h-10 text-slate-400 hover:text-rose-50 transition-all flex items-center justify-center hover:bg-rose-50 rounded-full font-black text-left text-left"><FiX size={24} /></button>
          <h3 className="text-2xl font-black text-[#74045F] mb-8 tracking-tight text-left"> {editingDoc ? "แก้ไขข้อมูลเอกสาร" : "อัปโหลดไฟล์ใหม่"} </h3>
          <div className="space-y-6 text-left text-left text-left text-left">
            {!editingDoc && (
              <div onClick={() => fileInputRef.current.click()} className="border-2 border-dashed rounded-[2.5rem] p-10 flex flex-col items-center justify-center transition-all cursor-pointer border-purple-100 bg-purple-50/30 hover:border-[#74045F] text-left text-left">
                <input type="file" ref={fileInputRef} hidden onChange={handleFileChange} />
                <FiUploadCloud size={48} className="text-purple-200 text-left text-left" />
            ...
            <div className="text-left font-bold text-left text-left">
              <label className="text-[14px] font-black text-purple-300 uppercase tracking-widest ml-1 font-black text-left text-left text-left">หมวดหมู่</label>
            ...
            <button onClick={handleSubmit} className="w-full bg-[#74045F] hover:bg-[#5a034a] text-white font-black py-5 rounded-2xl shadow-xl flex items-center justify-center gap-2 uppercase tracking-widest transition-all text-left text-left text-left">ยืนยันการอัปโหลด</button>
          </div>
        </div>
      </div>
    );
...
function ProfileInput({ label, value, onChange }) {
  return (
    <div className="space-y-2 text-left block font-bold text-left text-left">
      <label className="text-[14px] font-black text-purple-300 uppercase tracking-widest ml-1 font-black text-left text-left text-left">{label}</label>
      <input 
        type="text" 
        value={value} 
