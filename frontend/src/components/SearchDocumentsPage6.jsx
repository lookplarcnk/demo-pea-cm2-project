...
function Navbar() {
  ...
  return (
    <nav className="bg-white shadow-md border-b border-gray-200 sticky top-0 z-40 text-left">
      ...
      {toggle && (
        <div className="md:hidden bg-white border-t border-gray-100 py-6 px-6 space-y-5 shadow-inner animate-in fade-in zoom-in-95 duration-300 origin-top text-left text-left text-left">
...
// --- ส่วนที่ 2: หน้าหลักการค้นหาเอกสาร (คำสั่งและประกาศ) (ปรับโทนม่วง #74045F) ---
function SearchDocumentsPage6() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8] font-sans text-left flex flex-col text-left">
      <Navbar />
      {/* 🎨 เปลี่ยนพื้นหลัง Header เป็นสีม่วง PEA (#74045F) */}
        ...
        <section className="mt-16 text-left text-left text-left text-left text-left">
          {documents.length > 0 ? (
            <div className="results-container text-left text-left text-left text-left text-left">
              {/* 🎨 เปลี่ยนสีเส้นขอบข้างหัวข้อเป็นสีม่วง PEA */}
                ...
                {documents.map((doc) => (
                  <div key={doc.doc_id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-md hover:shadow-xl hover:border-[#74045F] transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group text-left text-left text-left text-left text-left text-left">
                    <div className="flex items-center gap-5 flex-1 cursor-pointer text-left text-left text-left text-left text-left text-left" onClick={() => handleAccess(doc)}>
                      {/* 🎨 ไอคอนและพื้นหลังไอคอนเปลี่ยนเป็นม่วง PEA */}
            ...
            /* 🎨 เปลี่ยนสีกล่องสถานะเป็นม่วงอ่อน PEA */
