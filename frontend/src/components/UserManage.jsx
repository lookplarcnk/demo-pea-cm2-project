...
export default function UserManagementPage() {
    ...
    try {
      ...
      if (activeTab === "พนักงานภายใน") {
        ...
        const mappedPublicUsers = response.data.map(u => ({
          ...
          role: null,
          status: u.status || "ใช้งานอยู่",
          avatar: `https://ui-avatars.com/api/?name=${u.first_name || 'P'}&background=6366f1&color=fff&rounded=true`,
          canApprove: false,
          joinDate: new Date(u.created_at || Date.now()).toISOString().split('T')[0]
        }));
  ...
  const getDeptColor = (dept) => {
    if (!dept) return "bg-slate-50 text-slate-600 border-slate-100";
