              ...
              <div className="relative text-left">
                <button onClick={() => setOpenProfile(!openProfile)} className="flex items-center gap-2 outline-none text-left">
                  <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center overflow-hidden border border-purple-200 text-left">
                    {user.avatar ? <img src={user.avatar} alt="avatar" className="w-full h-full object-cover text-left" /> : <FiUser className="text-[#74045F] text-xl text-left" />}
                  </div>
                  <span className="text-sm font-bold text-gray-700 text-left">{user.firstName}</span>
                </button>
                {openProfile && (
                  <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-200 py-2 animate-in fade-in zoom-in duration-150 text-left text-left">
...
// --- ส่วนที่ 2: หน้าหลักการค้นหาเอกสาร (รายงานประจำปี) (ปรับโทนม่วง #74045F) ---
function SearchDocumentsPage5() {
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8] font-sans text-left flex flex-col text-left">
      <Navbar />
      {/* 🎨 เปลี่ยนสี BG Header เป็นสีม่วง PEA (#74045F) */}
      <header className="bg-[#74045F] py-16 px-4 border-b-4 border-[#74045F] text-left">
      ...
      <main className="container mx-auto max-w-[950px] px-4 -mt-12 pb-24 text-left flex-grow text-left">
        <div className="bg-white p-6 md:p-10 rounded-3xl shadow-2xl border border-gray-100 text-left relative text-left">
          ...
          <form className="space-y-8 text-left mt-8 text-left" onSubmit={handleSearch}>
            ...
            {/* 🎨 เปลี่ยนสีเมนูเพิ่มเติมเป็นสีม่วง PEA */}
            <button type="button" onClick={() => setShowAdvance(!showAdvance)} className="flex items-center gap-2 text-sm font-bold text-[#74045F] hover:text-[#5a034a] transition-colors text-left text-left text-left">
            ...
            {/* 🎨 เปลี่ยนสีปุ่มค้นหาเป็นสีม่วง PEA */}
            <button type="submit" className="w-full bg-[#74045F] text-white py-4.5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:bg-[#5a034a] transition-all shadow-xl shadow-purple-900/10 active:scale-[0.98] text-center">
              <FiSearch size={22} className="text-left" /> {loading ? "กำลังค้นหา..." : "ค้นหารายงานตอนนี้"}
            </button>
          </form>
        </div>
        ...
        <section className="mt-16 text-left text-left text-left text-left">
          {documents.length > 0 ? (
            <div className="results-container text-left text-left text-left text-left">
              {/* 🎨 เปลี่ยนสีเส้นขอบข้างหัวข้อรายการเป็นสีม่วง PEA */}
              <p className="text-gray-400 font-black text-xs uppercase tracking-[0.2em] mb-8 border-l-4 border-[#74045F] pl-4 text-left text-left text-left">รายงานที่เกี่ยวข้อง ({documents.length})</p>
                ...
                {documents.map((doc) => (
                  <div key={doc.doc_id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-md hover:shadow-xl hover:border-[#74045F] transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group text-left text-left text-left text-left text-left">
                    <div className="flex items-center gap-5 flex-1 cursor-pointer text-left text-left text-left text-left text-left" onClick={() => handleAccess(doc)}>
                      ...
                      <div className="text-left text-left text-left text-left text-left">
                        <h4 className="font-black text-gray-800 text-lg group-hover:text-[#74045F] transition-colors flex items-center gap-2 text-left text-left text-left text-left text-left">
                          {doc.doc_name}
                          {doc.require_login && <span className="text-[10px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-bold uppercase flex items-center gap-1 text-left shadow-sm"><FiLock size={10} className="text-left" /> Staff Only</span>}
                        </h4>
                        <div className="flex flex-wrap gap-x-5 gap-y-2 mt-2 text-[11px] font-black text-gray-400 uppercase tracking-widest text-left text-left text-left text-left text-left">
                          ...
                          <span className="text-[#74045F] font-bold text-left">{doc.category || "รายงาน"}</span>
                    ...
                    <div className="flex items-center gap-3 text-left">
                      {/* 🎨 ปุ่มเปิดดูรายงานเป็นม่วง PEA */}
                      <button 
                        onClick={() => handleAccess(doc)} 
                        className="p-3 bg-gray-50 text-gray-400 hover:text-[#74045F] hover:bg-purple-50 rounded-xl transition-all shadow-sm text-left text-left text-left" 
            ...
            /* 🎨 เปลี่ยนสีกล่องสถานะว่างเปล่าเป็นม่วง PEA */
