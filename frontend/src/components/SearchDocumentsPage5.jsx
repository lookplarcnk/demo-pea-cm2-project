...
function Navbar() {
  ...
  // กรองเฉพาะบุคคลทั่วไป (ไม่ดึงแอดมินมาแสดงตามเงื่อนไขที่คุณต้องการ)
    ...
    <nav className="bg-white shadow-md border-b border-gray-200 sticky top-0 z-40 text-left">
      <div className="container mx-auto max-w-[1320px] px-4 md:px-6 text-left">
        <div className="flex items-center justify-between h-[85px] text-left">
          ...
          {/* Hamburger Button พร้อม Animation */}
...
// --- ส่วนที่ 2: หน้าหลักการค้นหาเอกสาร (รายงานประจำปี) ---
function SearchDocumentsPage5() {
  ...
  // ดึงข้อมูลรายชื่อแผนกจาก Database
  useEffect(() => {
        ...
        <section className="mt-16 text-left text-left text-left text-left">
          {documents.length > 0 ? (
            <div className="results-container text-left text-left text-left text-left">
              <p className="text-gray-400 font-black text-xs uppercase tracking-[0.2em] mb-8 border-l-4 border-[#00BEEA] pl-4 text-left text-left text-left">รายงานที่เกี่ยวข้อง ({documents.length})</p>
                    ...
                    <div className="flex items-center gap-5 flex-1 cursor-pointer text-left text-left text-left text-left text-left" onClick={() => handleAccess(doc)}>
                      ...
                      <div className="text-left text-left text-left text-left text-left">
                        <h4 className="font-black text-gray-800 text-lg group-hover:text-[#00BEEA] transition-colors flex items-center gap-2 text-left text-left text-left text-left text-left">
                          {doc.doc_name}
                          {doc.require_login && <span className="text-[10px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-bold uppercase flex items-center gap-1 text-left"><FiLock size={10} className="text-left" /> Staff Only</span>}
                        </h4>
                        <div className="flex flex-wrap gap-x-5 gap-y-2 mt-2 text-[11px] font-black text-gray-400 uppercase tracking-widest text-left text-left text-left text-left text-left">
                          <span className="text-[#00BEEA] font-bold text-left">{doc.category || "รายงาน"}</span>
