...
import Footer from "./Footer"; // นำเข้า Footer จากไฟล์ภายนอก
...
function SearchDocumentsPage5() {
   ...
   * รองรับชื่อไฟล์ภาษาไทยผ่านเส้นทาง /files/ ของ Backend
   */
    ...
    <div className="min-h-screen bg-[#F0F4F8] font-sans text-left flex flex-col">
      ...
      <header className="bg-[#00BEEA] py-16 px-4 border-b-4 border-[#00BEEA]">
        <div className="container mx-auto max-w-[1200px] text-center">
          <h2 className="text-2xl md:text-4xl font-black text-white mb-2 uppercase tracking-wide">ค้นหาเอกสารรายงานประจำปี</h2>
          <p className="text-white opacity-90 text-sm md:text-base font-bold">สรุปผลการดำเนินงาน ข้อมูลสถิติ และรายงานความยั่งยืน การไฟฟ้าส่วนภูมิภาคจังหวัดเชียงใหม่ 2</p>
        </div>
      </header>
        ...
        <div className="bg-white p-6 md:p-10 rounded-3xl shadow-2xl border border-gray-100 text-left">
          <form className="space-y-8" onSubmit={handleSearch}>
            ...
            <button type="submit" className="w-full bg-[#004B8D] text-white py-4.5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 hover:bg-[#003366] transition-all shadow-xl shadow-blue-900/10 active:scale-[0.98]">
              <FiSearch size={22} /> {loading ? "กำลังค้นหา..." : "ค้นหารายงานตอนนี้"}
            </button>
          </form>
        </div>
        ...
        <section className="mt-16 text-left">
          {documents.length > 0 ? (
            <div className="results-container">
              <p className="text-gray-400 font-black text-xs uppercase tracking-[0.2em] mb-8 border-l-4 border-[#00BEEA] pl-4 text-left">รายงานที่เกี่ยวข้อง ({documents.length})</p>
                    ...
                    <div className="flex items-center gap-5 flex-1 cursor-pointer" onClick={() => handleAccess(doc)}>
                      ...
                      <div className="text-left">
                        <h4 className="font-black text-gray-800 text-lg group-hover:text-[#00BEEA] transition-colors flex items-center gap-2">
                          {doc.doc_name}
                          {doc.require_login && <span className="text-[10px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-bold uppercase">Staff Only</span>}
                        </h4>
                        <div className="flex flex-wrap gap-x-5 gap-y-2 mt-2 text-[11px] font-black text-gray-400 uppercase tracking-widest text-left">
                          <span className="text-[#00BEEA]">{doc.category || "รายงาน"}</span>
