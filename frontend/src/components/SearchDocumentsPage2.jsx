              ...
              <div className="relative text-left">
                ...
                {openProfile && (
                  <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-200 py-2">
                    ...
                    <button logout={logout} className="w-full text-left px-4 py-2 text-sm font-bold text-red-600 hover:bg-red-50 flex items-center gap-2">
                      <FiLogOut /> ออกจากระบบ
                    </button>
                  </div>
                )}
...
function SearchDocumentsPage2() {
  ...
  /**
   * ส่วนที่แก้ไข: เปิดเอกสารฉบับจริง (Preview) ใน Tab ใหม่
   * ปรับปรุง Logic การสร้าง URL ให้ตรงกับไฟล์ภาษาไทยในโฟลเดอร์ uploads
   */
  const handleAccess = (doc) => {
    if (doc.require_login && !user) {
      ...
      /**
       * อ้างอิงจากข้อมูลโครงสร้างและรูปภาพ:
       * 1. ฐานข้อมูลเก็บชื่อจริงไว้ใน doc_name
       * 2. ไฟล์จริงในโฟลเดอร์ uploads ตั้งชื่อตาม doc_name และมีนามสกุล .pdf
       */
      const fileName = encodeURIComponent(`${doc.doc_name}.pdf`);
      
      // เรียกผ่านเส้นทางเสมือน /files/ ที่ Backend map ไว้
      const fileUrl = `http://localhost:5000/files/${fileName}`;
      
      // เปิดใน Tab ใหม่เพื่อให้ Browser แสดงผล Preview ทันที
    ...
    <div className="min-h-screen bg-[#F0F4F8] font-sans text-left flex flex-col">
      ...
      <header className="bg-[#00BEEA] py-16 px-4 border-b-4 border-[#00BEEA]">
        <div className="container mx-auto max-w-[1200px] text-center">
          <h2 className="text-2xl md:text-4xl font-black text-white mb-2 uppercase tracking-wide">ค้นหาคู่มือและ SOP</h2>
          <p className="text-white opacity-90 text-sm md:text-base font-bold">คลังความรู้และมาตรฐานการทำงาน การไฟฟ้าส่วนภูมิภาคจังหวัดเชียงใหม่ 2</p>
        </div>
      </header>
        ...
        <section className="mt-16 text-left">
          {documents.length > 0 ? (
            <div className="results-container">
              <p className="text-gray-400 font-black text-xs uppercase tracking-[0.2em] mb-8 border-l-4 border-[#00BEEA] pl-4 text-left">
                  ...
                  <div key={doc.doc_id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-md hover:shadow-xl hover:border-[#00BEEA] transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group">
                    <div className="flex items-center gap-5 flex-1 cursor-pointer" onClick={() => handleAccess(doc)}>
                      ...
                      <div className="text-left">
                        <h4 className="font-black text-gray-800 text-lg group-hover:text-[#00BEEA] transition-colors flex items-center gap-2">
                          {doc.doc_name}
                          {doc.require_login && <span className="text-[10px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full">ต้องล็อกอิน</span>}
                        </h4>
                    ...
                    <div className="flex items-center gap-3">
                      <button 
                        ...
                        className="p-3 bg-gray-50 text-gray-400 hover:text-[#00BEEA] hover:bg-[#E0F2FE] rounded-xl transition-all" 
